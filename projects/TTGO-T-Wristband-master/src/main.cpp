...
void setup(void)
{
  ...
  attachInterrupt(CHARGE_PIN, [] {charge_indication = true;}, CHANGE);
  ...
  targetTime = millis() + 1000;
  for(int i=0;i<Timernum;i++)t[i].reset();
  sleeptime=30;
  wifiscansetup=false;
  wifiscantask=false;
...
void weather_show()
{
  if(t[3].stand_by(0.5))
  {
    ...
    if(!httperror)
    {
      ...
      if(now_weather=="Clear")drawmark(&tft,0,t,TFT_YELLOW,140,16);
      else if(now_weather=="Clouds")drawmark(&tft,1,t,TFT_DARKGREY,140,16);
      else if(now_weather=="Rain")drawmark(&tft,2,t,TFT_BLUE,140,16);
      else if(now_weather=="Mist")drawmark(&tft,3,t,TFT_SILVER,140,16);
      else if(tom_weather=="Snow")drawmark(&tft,4,t,TFT_WHITE,140,16);
      tft.drawString("tomorrow:"+String(tom_weather)+" "+String(tom_temp)+"[C]   ",0,32);
      if(tom_weather=="Clear")drawmark(&tft,0,t,TFT_YELLOW,140,32);
      else if(tom_weather=="Clouds")drawmark(&tft,1,t,TFT_DARKGREY,140,32);
      else if(tom_weather=="Rain")drawmark(&tft,2,t,TFT_BLUE,140,32);
      else if(tom_weather=="Mist")drawmark(&tft,3,t,TFT_SILVER,140,32);
      else if(tom_weather=="Snow")drawmark(&tft,4,t,TFT_WHITE,140,32);
      t++;
      if(t>3)t=0;
    }
    else
...
void RTC_Show()
{
    ...
    {   
        ...
        {
          int color=TFT_BLACK;
          if(now_weather=="Clear")      {color=TFT_YELLOW;drawmark(&tft,0,t,TFT_YELLOW,x+mx,y);}
          else if(now_weather=="Clouds"){color=TFT_DARKGREY;drawmark(&tft,1,t,TFT_DARKGREY,x+mx,y);}
          else if(now_weather=="Rain")  {color=TFT_BLUE;drawmark(&tft,2,t,TFT_BLUE,x+mx,y);}
          else if(now_weather=="Mist")  {color=TFT_SILVER;drawmark(&tft,3,t,TFT_SILVER,x+mx,y);}
          else if(tom_weather=="Snow")  {color=TFT_WHITE;drawmark(&tft,4,t,TFT_WHITE,x+mx,y);}
...
void comp()
{
  if(t[3].stand_by(0.2))
  {
    ...
    const char* s="N";
    if(337.5<Bearing||Bearing<22.5)      s="N";
    else if(22.5<Bearing&&Bearing<67.5)  s="NE";
    else if(67.5<Bearing&&Bearing<112.5) s="E";
    else if(112.5<Bearing&&Bearing<157.5)s="SE";
    else if(157.5<Bearing&&Bearing<202.5)s="S";
    else if(202.5<Bearing&&Bearing<247.5)s="SW";
    else if(247.5<Bearing&&Bearing<292.5)s="W";
    else if(292.5<Bearing&&Bearing<337.5)s="NW";
...
void loop()
{
    ...
    if (!otaStart)
    {
      ...
      else 
      {
          ...
          if(t[0].stand_by(sleeptime)&&wifisetup==false)func_select=funcnum;
          pressed = false;
      }
